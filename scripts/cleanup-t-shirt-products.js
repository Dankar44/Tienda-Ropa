...
async function cleanupTShirtProducts() {
    try {
        console.log('🔄 Iniciando limpieza de productos t-shirt...');

        // Primero, verificar cuántos productos t-shirt existen
        const countResult = await query(`
            SELECT COUNT(*) as count 
            FROM products 
            WHERE garment_type = 't-shirt'
        `);
        
        const productCount = countResult.rows[0].count;
        console.log(`📊 Productos t-shirt encontrados: ${productCount}`);

        if (productCount === 0) {
            console.log('✅ No hay productos t-shirt para eliminar');
            return;
        }
        ...
        console.log(`✅ Eliminados ${deleteResult.rowCount} productos t-shirt de la base de datos`);
        ...
        console.log(`📊 Productos t-shirt restantes: ${finalCount}`);

        if (finalCount === 0) {
            console.log('🎉 Limpieza completada exitosamente');
        } else {
            console.log('⚠️  Aún quedan productos t-shirt en la base de datos');
        }
...
module.exports = { cleanupTShirtProducts };
